    ...
    for page in range(1, pageNum + 1):
        ...
        try:
            ...
            for hotelInfo in hotelList:
                hotelSummaryList.append(hotelInfo['name'] + '|' + hotelInfo['score'] + '|' + hotelInfo['dpcount'] + '|' + hotelInfo['dpscore'] + '|' + hotelInfo['stardesc'] + '|' + hotelInfo['address'] + '|' + hotelInfo['id'] + '|' + BaseUrl + hotelInfo['url'])
                print(hotelInfo)
...
idList = getCityId()
for item in idList:
    pageNum = getPageNum(item.split('|')[1])
    print(pageNum)
